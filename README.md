...
Connect the introduction to a reader's setup, such as italic comments, narrow editor panes, or a Powerline prompt. Name a visible difference or similarity before inviting readers to inspect it; avoid paragraphs that could describe any font pair. Keep technical measurement terms in the notes. Recommendations should identify one or two relevant reasons to choose each font. Comparison tables report features without a “Better fit” or winner column: more weights, axes, or supported languages do not automatically make a font the better choice for a particular reader.

Every comparison post must include a summary infographic before publication, in addition to its detailed specimens. Put it in a collapsed `<details>` element after the comparison table and before Measurement notes, with `<summary>View comparison infographic</summary>`. Use an `<img>` referencing `/images/comparison-monolisa-vs-<typeface>-summary.svg`, with alt text naming the compared fonts and `width="100%"`. Omit the `open` attribute so the graphic is optional in the reading flow. The infographic must match the post's measured font version and scope; explain any coverage or speaker-count estimates alongside it inside the disclosure. Retain the infographic when rewriting the prose. Berkeley Mono is an editorial exception: its redundant infographic was removed and the information consolidated into the comparison table.
...
Where the copy identifies a particular letterform difference, add an enlarged detail with a soft accent-colored circle behind the relevant part of each font's glyph. Use brief, neutral captions and retain the complete specimen below. The detail pairs stay adjacent on mobile. Highlight definitions live in `scripts/comparison-focus.mjs`; regenerate the affected desktop and mobile SVGs with `node scripts/render-comparison-svgs.mjs scripts/comparison-fonts.local.json --focus-only`. Add a font key before `--focus-only` to render one comparison. These details use actual font outlines at equal nominal sizes, with separate upright/italic files where available. Fira Code's comparison explicitly labels its software slant: a 10° shear of the upright font, matching the angle recorded in MonoLisa's italic file. Use the same synthesis in the enlarged details and full code specimen, and document the angle in Measurement notes. Berkeley keeps its documented independently enlarged vendor outlines and uses `scripts/render-berkeley-details.mjs`.

Terminal graphics follow the landing page's “Symbols for Terminal” treatment: matching terminal windows with colored Powerline prompt segments, a box-drawing table, standard Unicode block progress bars, and a compact status line. Both fonts use identical content, size, line height, and theme colors. Render all symbols from the measured font files, including the separators; do not draw replacement arrows or use a fallback font for missing glyphs. Use standard blocks rather than MonoLisa's private-use progress symbols for this shared coverage comparison. Stack the windows on mobile. Regenerate with `node scripts/render-comparison-svgs.mjs scripts/comparison-fonts.local.json --terminal-only`; the layout lives in `scripts/render-terminal-comparison.mjs`. Apply this format to deferred comparisons once their licensed font files are available.
