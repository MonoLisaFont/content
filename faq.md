...
[Note that there's an upgrade path with discounts available.](https://www.monolisa.dev/orders)

</Details>

<Details>
### How can I access version 1 or 2?
...
Yes, [there’s a limited trial available](https://www.monolisa.dev/buy/trial). [See the tester](https://www.monolisa.dev/tester) as well if you don't want to install the typeface on your computer.
...
- Trial – Regular (`400`), Bold (`700`). The plan is available in ttf.
- Developer – Thin (`100`), ExtraLight (`200`), Light (`300`), Regular (`400`), Medium (`500`), SemiBold (`600`), Bold (`700`), ExtraBold (`800`), Black (`900`). On top of this, the Developer version is available in a variable format (no fixed weight). The plan is available in both ttf and woff2 for web usage.
- Creator - Any of the above and you can choose which ones to license.

You can [download your purchases package at the orders page](https://www.monolisa.dev/orders).

<aside>
💡 Note that the Developer and Creator plans contain more alternative glyphs and have access to the customize tool that makes it easy to generate custom versions of the typeface even per application. This way you can avoid software limitations and use exactly the features you want.
</aside>
</Details>
...
[Hyperglot 0.8.1](https://github.com/rosettatype/hyperglot) reports that MonoLisa supports 593 primary orthographies for living languages across five scripts:

- **Latin (496):** English, Spanish, French, German, Portuguese, Italian, ... and more
- **Cyrillic (88):** Russian, Ukrainian, Bulgarian, Serbian, ... and more
- **Hebrew (5):** Hebrew, Eastern Yiddish, Western Yiddish, Bukharic, Judeo-Persian
...
1. Make sure both normal and italic versions are installed to your system. It’s the italic version that includes ss01.
2. If you use VS Code, make sure to use the non-customized version (i.e., download without customizing). You can find more specific instructions below at the VS Code section on how to do this and how to enable ss01 then.
3. If you use an editor that doesn’t let you enable ss01 (IntelliJ etc.), please use the customize tool to freeze it to the font itself at your order page.
4. To see ss01 in action, make sure to use text with italics applied to it as otherwise it won’t render. Depending on the editor, you may have to change the font for comments or the code feature where you want to use ss01.
...
```json
{
  "editor.fontLigatures": "'calt' on, 'liga' off, 'zero' on, 'ss01' on"
}
```
...
# another way
font_family family="MonoLisaCode"
bold_font auto
italic_font auto
